...
if __name__ == "__main__":
    ...
    doDeblending = True 
    baseOutDir = 'singleAmpMag/'
    starSep = 5 # then they're definitely overlappig
    for mag in [16,15,14,13,12,11,10]:
        magList = [magVal, mag]
        print('\nStarting ccLoop for %d %% sep, using stars with %d and %d mag'%(starSep, magVal, mag))
        
        outputDir = baseOutDir+testLabel+'_%d' % mag
        print('The outputDir is ', outputDir)

        testName  = '%s.%d' % (testLabel, mag)
        print('The testName is %s'%testName)

        skyFilePath = baseOutDir+'starCat_%d.txt'%mag
        ...
        # if re-using the flats and calibs : copy from the sep_10  ..
        if not args.opd and not args.flats : 
            print('Copying content of /singleAmp/sep_10/ to re-use the flats and OPD files...')
